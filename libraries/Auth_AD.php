		...
		if (strlen($username) > 0 && strlen($password) > 0)
		{
			...
			if ($continue)
			{
				...
				if ($search = ldap_search($this->_ldap_conn, $this->_base_dn, $filter, $req_attrs))
				{
					...
					{
						log_message('error', 'Auth_AD: Unable to get entries for user.');
						show_error('Unable to read the AD entries for the user');
						$continue = false;
					}
				}
				else 
				{
					log_message('error', 'Auth_AD: Unable to perform search for user.');
					show_error('Unable to search the AD for the user');
					$continue = false;
				}
			}
			...
			if ($continue)
			{
				...
				if (!$bind = ldap_bind($this->_ldap_conn, $binddn, $password)) 
				{
